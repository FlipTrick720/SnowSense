	...
	public void scrapeSkiResortStatusForAllResorts() {
		try (Playwright playwright = Playwright.create()) {
			Browser browser = playwright
				...
				.launch(
					new BrowserType.LaunchOptions()
						.setHeadless(true)
						.setArgs(
							java.util.List.of("--ignore-certificate-errors")
						)
				);
			...
			for (SkiResort resort : resorts) {
				...
				try {
					bergfexInfrastructureStatus = new BergfexSkiResortSite(
						resort
					).scrapeAllInfrastructureStatus(page);
				} catch (TimeoutError e) {
					System.out.println(
						"Timeout waiting for status table for ski resort: " +
							resort.getName()
					);
					continue;
				}
				...
				for (SkiResortLift lift : bergfexInfrastructureStatus.getLifts()) {
					SkiResortLift existingLift = _skiResortLiftRepository
						.findAll()
						.stream()
						.filter(l ->
							l.getSkiResort().getId().equals(resort.getId())
						)
						.filter(l ->
							l.getName().equalsIgnoreCase(lift.getName())
						)
						.findFirst()
						.orElse(null);
					if (
						existingLift != null &&
						existingLift.getIsOpen() != lift.getIsOpen()
					) {
						existingLift.setIsOpen(lift.getIsOpen());
						existingLift.setLastStatusChange(
							lift.getLastStatusChange()
						);
				...
				for (SkiResortSlope slope : bergfexInfrastructureStatus.getSlopes()) {
					SkiResortSlope existingSlope = _skiResortSlopeRepository
						.findAll()
						.stream()
						.filter(s ->
							s.getSkiResort().getId().equals(resort.getId())
						)
						.filter(s ->
							s.getName().equalsIgnoreCase(slope.getName())
						)
						.findFirst()
						.orElse(null);
					if (
						existingSlope != null &&
						existingSlope.getIsOpen() != existingSlope.getIsOpen()
					) {
						existingSlope.setIsOpen(slope.getIsOpen());
						existingSlope.setLastStatusChange(
							slope.getLastStatusChange()
						);
